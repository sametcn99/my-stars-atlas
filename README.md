...
update the repository secrets or environment variables for your own username and token, then run the generator on your
fork.
...
You can then generate the data with either environment variables or CLI arguments:

```bash
bun run src/index.ts --username <your-github-handle>
```

Optional environment variables:

- `GITHUB_USERNAME` or `STARS_USERNAME`: the GitHub account whose stars will be indexed.
- `GITHUB_TOKEN` or `GH_TOKEN`: recommended to avoid low unauthenticated API limits.
- `README_TITLE`: custom README title.
- `README_DESCRIPTION`: custom README description.
